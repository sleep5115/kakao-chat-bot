...
- 등록 데이터에는 방 이름, 사용자 ID나 메시지 내용을 저장하지 않음
...
- 대화 내용, 방 이름, 닉네임과 사용자 ID는 저장하지 않는다.
- 등록된 방의 `chat_id`, 방 유형과 등록 시각만 등록 기간 동안 보관한다.
- 등록 완료 직후 정책을 별도 말풍선으로 고지하며 `!봇정보`로 다시 확인할 수 있다.
- 등록된 방에서 `!봇해제`를 입력하면 해당 방의 등록 정보를 즉시 삭제한다.

...
## Discord에서 카카오톡으로 보내기

Discord 전용 봇의 `/카톡` 명령으로 등록된 카카오톡 방 하나에 메시지를 보낼 수
있습니다. Discord 화면에서 `/카톡`을 선택한 뒤 `메시지` 입력칸에 내용을 넣으면
카카오톡에는 다음 형식으로 전송됩니다.

```text
디코에서 디스코드닉네임님이 메시지를 보냈습니다 : 입력한 내용
```

명령은 설정된 Discord 길드에서만 동작합니다. `DISCORD_CHANNEL_ID`를 지정하면
특정 채널로 더 좁힐 수 있고, 사용자/역할 ID 허용 목록도 선택적으로 적용할 수
있습니다. Discord 서비스는 카카오봇과 별도 컨테이너로 실행되며 공개 포트를
열지 않고 Docker 내부 API로 카카오봇에 전달합니다.
